...
function App() {
    ...
    (variant: MUI.ButtonProps['variant']) => (
      <>
        <MUI.Stack direction='row' spacing={1}>
          ...
          <LUI.Button variant={variant} loadingProgress={0} tooltip='This is loading'>
            Button
          </LUI.Button>
  ...
  const renderSelects = useCallback((variant: MUI.SelectProps['variant']) => {
    const options = [
      ...
      { value: '3', label: 'Icon', icon: <LUI.Icon name='mdi-bike' size='tiny' /> }
    ];
    return (
      <>
        <MUI.Stack direction='row' spacing={1} alignItems='flex-end'>
          <LUI.Select variant={variant} size='tiny' value='1' options={options} />
          <LUI.Select variant={variant} size='small' value='1' options={options} />
          <LUI.Select variant={variant} size='medium' value='1' options={options} />
        </MUI.Stack>
      ...
      return (
        <>
          <MUI.Stack direction='row' alignItems='flex-end' spacing={2}>
            ...
            <LUI.TextInput tooltip='Error text' variant={variant} error value='oh no!' />
          </MUI.Stack>

          <MUI.Stack direction='column' spacing={2} style={{ width: '500px' }}>
            <LUI.TextElement name='A text element' textInputProps={{ variant, value: textValue }} />
        ...
        <>
          ...
          <MUI.Stack direction='column' spacing={2} style={{ width: '500px' }}>
            ...
            <LUI.NumberDuoElement
              ...
              inputProps1={{ variant, value: numberValue, min: 5, max: 15, tooltip: 'First value' }}
              inputProps2={{ variant, value: 20, min: 10, decimals: 3, tooltip: 'Second value' }}
            />
          </MUI.Stack>
        </>
      );
    ...
    <MUI.Stack spacing={2}>
      ...
      <Group title='<BrowseFile />'>
        <MUI.Stack direction='column' spacing={2} style={{ width: '600px' }}>
          ...
          <LUI.BrowseFileElement name='Missing file' value='C:/some/fake/path' fileNotFound />
      ...
      <Group title='<ColorPicker />'>
        ...
        <LUI.ColorElement
          ...
          pickerProps={useMemo(() => ({ value: { r: 255, g: 0, b: 128 }, variant: 'compact' }), [])}
        />
      </Group>
      ...
      <Group title='<DialogOverlay />'>
        <LUI.Button variant='outlined' onClick={() => setShowDialogOverlay1(!showDialogOverlay1)}>
        ...
        <LUI.Button variant='outlined' onClick={() => setShowDialogOverlay2(!showDialogOverlay2)}>
          Show dialog without title
        </LUI.Button>
        <LUI.DialogOverlay
          open={showDialogOverlay2}
          actions={[{ label: 'Close', action: () => setShowDialogOverlay2(false) }]}
          onClose={() => setShowDialogOverlay2(false)}
        >
          <MUI.Typography>Hello there</MUI.Typography>
        </LUI.DialogOverlay>

        <LUI.Button variant='outlined' onClick={() => setShowDialogOverlay3(!showDialogOverlay3)}>
