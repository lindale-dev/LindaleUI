...
export default window.skpCallback = function(callbackName: string, params?: any)
{
    console.info(`skpCallback: ${callbackName}, ${JSON.stringify(params)}`);

    if (typeof(callbackName) !== 'string')
    {
        console.error(`skpCallback: arg 0 invalid, type is ${typeof(callbackName)}, must be string`);
        return;
    }
