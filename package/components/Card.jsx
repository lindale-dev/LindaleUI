...
class Card extends React.Component {
  ...
  render() {
    ...
    const expandCallback = this.props.expandable ? this.expand.bind(this) : null;

    const expandIcon = this.props.expandable ? (
      <Icon
        icon={this.state.expanded ? 'mdi-chevron-down' : 'mdi-chevron-left'}
        className={classes.cardTitleIcon}
        size={18}
        color={this.props.theme.palette.text.secondary}
      />
    ...
    const contentHeight = this.state.expanded ? 'auto' : 0;

    return (
      <div className={`${classes.card} ${this.props.className}`} ref={this.props.cardRef}>
        ...
        <AnimateHeight duration={200} height={contentHeight}>
          <div className={classes.cardContent}>{this.props.children}</div>
        </AnimateHeight>
      </div>
    );
  }
